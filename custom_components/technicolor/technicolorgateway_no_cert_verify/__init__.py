...
from custom_components.technicolor.technicolorgateway_no_cert_verify import mysrp as srp
from custom_components.technicolor.technicolorgateway_no_cert_verify.modal import get_device_modal, get_broadband_modal
...
class TechnicolorGateway:
    def __init__(self, host, port, user, password) -> None:
        ...
        self._br = RoboBrowser(history=True, parser="html5lib")
    ...
    def srp6authenticate(self):
        ...
        self._br.open(f'{self._uri}/authenticate', method='post',
                      data=urlencode({'CSRFtoken': token,
                                      'I': uname, 'A': binascii.hexlify(A)}), verify=False)
        ...
        self._br.open(f'{self._uri}/authenticate', method='post',
                      data=urlencode({'CSRFtoken': token, 'M': binascii.hexlify(M)}), verify=False)
        _LOGGER.debug("br.response %s", self._br.response)
        j = json.decoder.JSONDecoder().decode(self._br.parsed.decode())
    ...
    def get_device_modals(self, device_modal):
        ...
        req = self._br.session.get(device_modal, verify=False)
    ...
    def get_broadband_modal(self):
        req = self._br.session.get(f"{self._uri}/modals/broadband-modal.lp", verify=False)
        self._br._update_state(req)
        content = req.content.decode()
